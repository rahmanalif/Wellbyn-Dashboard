...
import { useSubmitPatientRegistrationStep1Mutation } from "@/lib/store/services/patientsApi";
...
const PatientInfoPage = () => {
  ...
  const [submitPatientRegistrationStep1, { isLoading, error }] =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.numberOfChildren) formDataToSend.append("numberOfChildren", formData.numberOfChildren);
      if (formData.addressLine2) formDataToSend.append("addressLine2", formData.addressLine2);
      if (formData.employer) formDataToSend.append("employer", formData.employer);
      if (formData.driversLicense) formDataToSend.append("driversLicense", formData.driversLicense);
      if (formData.ssnLast4) formDataToSend.append("ssnLast4", formData.ssnLast4);
      if (formData.bloodGroup) formDataToSend.append("bloodGroup", formData.bloodGroup);

      // Add files if they exist
      if (frontLicenseFile) {
        formDataToSend.append("frontLicenseImage", frontLicenseFile);
      }
      if (backLicenseFile) {
        formDataToSend.append("backLicenseImage", backLicenseFile);
      }

      // Submit the form
      const response = await submitPatientRegistrationStep1(formDataToSend).unwrap();

      console.log("Registration Step 1 Success:", response);

      // Store patientId in sessionStorage for next steps
      ...
      router.push("/patients/medical-info");
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-between pt-6">
              <button
                ...
                disabled={isLoading}
                className="bg-white cursor-pointer text-gray-600 px-6 py-3 rounded-lg shadow-md font-medium hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </button>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="bg-primary-500 cursor-pointer text-white px-6 py-3 rounded-lg font-medium hover:bg-primary-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {isLoading ? (
                  <>
                    <svg
